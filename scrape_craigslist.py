...
def find_davis_ads(indiv_ads):
	...
	for i, ad in enumerate(indiv_ads):
		...
		#pdb.set_trace()
		if (((location == '') and (('soquel' not in title.lower()) and ('ben lomond' not in title.lower()) and ('felton' not in title.lower()))) or \
			((location.lower() != 'soquel') and (location.lower() != 'ben lomond') and (location.lower() != 'felton'))) and \
